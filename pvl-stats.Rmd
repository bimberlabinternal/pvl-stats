...
.ChronicPhaseSetpoint <- function(df = NULL, minPoints = 4, minDaysDuration = 7) {
  if (is.null(df) || nrow(df) == 0) {
    ...
    return(list(df = df, summary = tibble()))
  }
  ...
  if (nrow(pre_art_df) >= minPoints &&
    ...
    summary_outcomes <- append(summary_outcomes, list(
      .AppendSummaryRow(id, date, "preArtSetpoint", numeric_value = setpoint, data_source = data_source),
      .AppendSummaryRow(id, date, "preArtSetpointPACF", numeric_value = pacf_val, data_source = data_source)
    ))
  }
  ...
  if (nrow(post_art_df) >= minPoints &&
    ...
    summary_outcomes <- append(summary_outcomes, list(
      .AppendSummaryRow(id, date, "atiSetpoint", numeric_value = ati_setpoint, data_source = data_source)
    ))
  }
  ...
  if (exists("setpoint") && !is.na(setpoint)) {
    gg <- gg +
  ...
  if (exists("ati_setpoint") && !is.na(ati_setpoint)) {
    gg <- gg +
