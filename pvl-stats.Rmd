    ...
    for (i in 1:nrow(setpoints_df)) {
      ...
      gg <- gg + 
        annotate("text", 
                 ...
                 hjust = 0, vjust = -0.5, color = color, size = 3, fontface="bold")
    }
  }
...
RunARTPipeline <- function(df, acute_window = 21) {
  ...
  # 3. On-ART Setpoint and ART suppression (entire on-ART period)
  if (nrow(onart_df) >= 3) {
    onart_df <- onart_df[order(onart_df$date, decreasing = FALSE), ]
    ...
    # Get time to decline from acute phase peak
    at_lod_rle <- rle(onart_df$result <= 1)
    days_before_consecutive_LOD <- at_lod_rle$lengths[1:which.max(at_lod_rle$values) - 1] # get all samples before the day
    first_dpi_at_consecutive_LOD <- onart_df[1, ]$timepostsivchallenge_dayspostinfection
    days_before_LOD_cutoff <- onart_df[days_before_consecutive_LOD + 1, ]$timepostsivchallenge_dayspostinfection
    if(!length(days_before_LOD_cutoff) == 0) {
      first_dpi_at_consecutive_LOD <- days_before_LOD_cutoff
    }
    
    summary_outcomes <- append(summary_outcomes, list(
      .AppendSummaryRow(id, date, "onArtSetpoint", numeric_value = onart_setpoint, data_source = data_source, window_start = onart_setpoint_window_start, window_end = onart_setpoint_window_end),
      .AppendSummaryRow(id, date, "dpiToART_LOD", numeric_value = first_dpi_at_consecutive_LOD, data_source = data_source)
    ))
    
    computed_metrics$onart_setpoint <- onart_setpoint
    computed_metrics$first_dpi_at_consecutive_LOD <- first_dpi_at_consecutive_LOD
  }
  ...
  onart_blips <- onart_df %>% 
    filter(timepostsivchallenge_dayspostinfection >= computed_metrics$first_dpi_at_consecutive_LOD) %>%
  ...
  if (!is.na(art_end) && nrow(postrebound_df) > 0) {
    ...
    postrebound_peak_df <- postrebound_df %>% filter(dpi <= (art_end + 21))
    peak_row <- postrebound_peak_df %>% filter(result == max(result, na.rm = TRUE)) %>% slice(1)
    postrebound_peak <- peak_row$result
    postrebound_day <- peak_row$dpi
    
    # Post-rebound setpoint (14+ days after ART release)
    postrebound_setpoint_df <- postrebound_df %>% filter(dpi > (art_end + 21))
    if (nrow(postrebound_setpoint_df) >= 3) {
      ...
      summary_outcomes <- append(summary_outcomes, list(
        ...
        .AppendSummaryRow(id, date, "postReboundPeak_pVL", numeric_value = postrebound_peak, data_source = data_source),
        .AppendSummaryRow(id, date, "postReboundPeak_day", numeric_value = postrebound_day, data_source = data_source)
      ))
      
      computed_metrics$postrebound_setpoint <- postrebound_setpoint
      computed_metrics$postrebound_peak <- list(value = postrebound_peak, day = postrebound_day)
    }
  }
...
CreateUnifiedTimeSeriesPlot <- function(df, metrics, has_art = FALSE, skip_outcome_annotations = FALSE, art_acute_window = 21) {
  if (is.null(df) || nrow(df) == 0) {
    return(ggplot() + 
           theme_void() + 
           annotate("text", x = 0.5, y = 0.5, label = "No Data Available", size = 6, fontface="bold"))
  }
    ...
    if (!is.na(phases$art_start)) {
      gg <- gg + 
        ...
        annotate("text", x = phases$art_start, y = min(df$result[df$result > 0], na.rm = TRUE),
                 label = "ART Initiation", angle = 90, vjust = 1.2, hjust = 0, color = "blue", size = 3.5, fontface="bold")
    }
    
    # ART release line
    if (!is.na(phases$art_end)) {
      gg <- gg + 
        geom_vline(xintercept = phases$art_end, linetype = "dashed", color = "darkred", linewidth = 0.8) +
        annotate("text", x = phases$art_end, y = min(df$result[df$result > 0], na.rm = TRUE),
                 label = "ART Release", angle = 90, vjust = 1.2, hjust = 0, color = "darkred", size = 3.5, fontface="bold")
    }
    
    if (!is.na(metrics$acute_burden)) {
      gg <- gg + 
        annotate("text", x = (phases$art_start / 2), y = (max(df$result) / (10^4)),
                 label = paste0("Acute burden:\n", round(metrics$acute_burden, 0), " copies/mL * days"),
                 vjust = -0.5, hjust = 0.5, color = "darkorange", size = 3.5, fontface="bold")
    }
    
    # First time to LOD
    if (!is.na(metrics$first_dpi_at_consecutive_LOD)) {
      gg <- gg + 
        geom_vline(xintercept = metrics$first_dpi_at_consecutive_LOD, linetype = "dashed", color = "darkgreen", linewidth = 0.8) +
        annotate("text", x = metrics$first_dpi_at_consecutive_LOD, y = min(df$result[df$result > 0], na.rm = TRUE),
                 label = "ART Suppression", angle = 90, vjust = 1.2, hjust = 0, color = "darkgreen", size = 3.5, fontface="bold")
    }
  }
  ...
  if (has_art && !is.null(metrics$phases)) {
    ...
    # Post-rebound setpoint (from ART end + 21 days to end of data)
    if (!is.null(metrics$postrebound_setpoint) && !is.na(phases$art_end)) {
      rebound_start <- phases$art_end + 21
      if (rebound_start < max_x) {
        gg <- gg +
          annotate("text", x = (rebound_start + max_x)/2, y = metrics$postrebound_setpoint,
                   ...
                   hjust = 0.5, vjust = -0.5, color = "purple", size = 3.2, fontface="bold")
        
        postrebound_data <- df %>% filter(dpi > (phases$art_end + 14)) %>% arrange(dpi)
        gg <- gg +
          geom_area(data = postrebound_data, aes(x = dpi, y = result), 
    ...
    if (!is.null(metrics$segmentation_raw) && !is.null(metrics$segmentation)) {
      ...
      if (!is.null(metrics$acute_burden) && !is.null(acute_data) && nrow(acute_data) > 0) {
        ...
        if (!is.na(acute_dpi_range$min_dpi) && !is.na(acute_dpi_range$max_dpi)) {
          gg <- gg +
            annotate("text", x = (acute_dpi_range$min_dpi + acute_dpi_range$max_dpi)/2, 
                     ...
                     label = paste0("Acute burden:\n", round(metrics$acute_burden, 0), " copies/mL * days"),
                     hjust = 0.5, vjust = -0.5, color = "darkorange", size = 3.2, fontface="bold")
        }
      }
        ...
        if (!is.na(chronic_dpi_range$min_dpi) && !is.na(chronic_dpi_range$max_dpi)) {
          gg <- gg +
            annotate("text", x = (chronic_dpi_range$min_dpi + chronic_dpi_range$max_dpi)/2, 
                     ...
                     hjust = 0.5, vjust = -0.5, color = "darkred", size = 3.2, fontface="bold")
        gg <- gg +
          geom_area(data = chronic_data, aes(x = dpi, y = result), 
                    fill = "darkred", alpha = 0.08, inherit.aes = FALSE)
        }
      }
  ...
  if (!is.null(metrics$acute_peak)) {
    ...
    gg <- gg +
      geom_point(aes(x = metrics$acute_peak$day, y = metrics$acute_peak$value),
                 shape = 23, fill = "darkorange", color = "darkorange", size = 4) +
      annotate("text", x = metrics$acute_peak$day, y = metrics$acute_peak$value,
               label = "Acute phase peak", color = "darkorange", vjust = -1.2, hjust = 0.5, size = 3.2, fontface = "bold")
  }
  
  # Add 100 copies rebound point for ART subjects
  if (has_art && !is.null(metrics$rebound_time_100)) {
    gg <- gg +
      geom_point(aes(x = metrics$rebound_time_100$day, y = metrics$rebound_time_100$value),
                 shape = 21, fill = "darkred", color = "darkred", size = 3.5) +
      annotate("text", x = metrics$rebound_time_100$day, y = metrics$rebound_time_100$value,
               label = "Time to rebound\n(> 100 copies)", vjust = -0.5, hjust = 0.5, 
    ...
    if (exists("acute_data") && nrow(acute_data) >= 2) {
      gg <- gg +
        geom_area(data = acute_data, aes(x = dpi, y = result), 
                  fill = "orange", alpha = 0.09, inherit.aes = FALSE)
    }
  }
  ...
  gg <- gg +
    geom_hline(yintercept = min_lod, linetype = "dotted", color = "black", alpha = 0.7) +
    annotate("text", x = max(df$dpi, na.rm = TRUE), y = min_lod,
             label = paste0("LOD (", min_lod, ")"), hjust = 1, vjust = -0.5, 
             color = "black", size = 3, fontface="bold")
  
  if (!is.null(metrics$postrebound_peak)) {
    peak_color <- if (has_art) "purple" else "purple"
    gg <- gg +
      geom_point(aes(x = metrics$postrebound_peak$day, y = metrics$postrebound_peak$value),
                 shape = 23, fill = "purple", color = "purple", size = 4) +
      annotate("text", x = metrics$postrebound_peak$day, y = metrics$postrebound_peak$value,
               label = "Post-rebound peak\n(first 3 weeks)", color = "purple", vjust = -1.2, hjust = 0.5, size = 3.2, fontface = "bold")
  }
