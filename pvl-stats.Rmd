...
.CreateSetpointPlot <- function(data_df, setpoints_df, top_2_runs) {
  ...
  if (nrow(setpoints_df) > 0) {
    ...
    for (i in 1:nrow(setpoints_df)) {
      ...
      gg <- gg + 
        geom_hline(yintercept = setpoint_info$setpoint, 
                   linetype = "solid", 
                   color = color, 
                   linewidth = 0.6, 
                   alpha = 0.8) +
...
RunARTPipeline <- function(df) {
  ...
  # Phase-specific data
  acute_df <- df %>% filter(dpi < art_start, dpi >= 0)
  onart_df <- df %>% filter(dpi >= art_start, if (!is.na(art_end)) dpi <= art_end else TRUE)
  ...
  # 4. On-ART Blip Frequency
  onart_blips <- onart_df %>% 
    filter(!is.na(result), !is.na(lod), result > lod)
  ...
  computed_metrics$onart_blips <- list(count = blip_count, data = onart_blips)
...
CreateUnifiedTimeSeriesPlot <- function(df, metrics, has_art = FALSE) {
  if (is.null(df) || nrow(df) == 0) {
    return(ggplot() + 
           theme_void() + 
  ...
  if (has_art && !is.null(metrics$phases)) {
    ...
    # Acute phase setpoint (from 0 to ART start)
    if (!is.null(metrics$acute_setpoint) && !is.na(phases$art_start)) {
      gg <- gg +
        geom_segment(x = 0, xend = max_x,
                     y = metrics$acute_setpoint, yend = metrics$acute_setpoint,
                     linetype = "solid", color = "orange", linewidth = 0.6, alpha = 0.8) +
    ...
    if (!is.null(metrics$onart_setpoint)) {
      ...
      gg <- gg +
        geom_segment(x = phases$art_start, xend = max_x,
                     y = metrics$onart_setpoint, yend = metrics$onart_setpoint,
                     linetype = "solid", color = "blue", linewidth = 0.6, alpha = 0.8) +
    ...
    if (!is.null(metrics$postrebound_setpoint) && !is.na(phases$art_end)) {
      ...
      if (rebound_start < max_x) {
        gg <- gg +
          geom_segment(x = rebound_start, xend = max_x,
                       y = metrics$postrebound_setpoint, yend = metrics$postrebound_setpoint,
                       linetype = "solid", color = "purple", linewidth = 0.6, alpha = 0.8) +
    ...
    if (!is.null(metrics$segmentation_raw) && !is.null(metrics$segmentation)) {
      ...
      metrics$acute_phase_data <- metrics$acute_phase_data %>% mutate(dpi = timepostsivchallenge_dayspostinfection)
  metrics$chronic_phase_data <- metrics$chronic_phase_data %>% mutate(dpi = timepostsivchallenge_dayspostinfection)
      ...
      if (!is.null(metrics$acute_setpoint) && !is.null(acute_data) && nrow(acute_data) > 0) {
        ...
        if (!is.na(acute_dpi_range$min_dpi) && !is.na(acute_dpi_range$max_dpi)) {
          gg <- gg +
            geom_segment(x = acute_dpi_range$min_dpi, xend = acute_dpi_range$max_dpi,
                         y = metrics$acute_setpoint, yend = metrics$acute_setpoint,
                         linetype = "solid", color = "blue", linewidth = 0.6, alpha = 0.8) +
      ...
      if (!is.null(metrics$chronic_setpoint) && !is.null(chronic_data) && nrow(chronic_data) > 0) {
        ...
        if (!is.na(chronic_dpi_range$min_dpi) && !is.na(chronic_dpi_range$max_dpi)) {
          gg <- gg +
            geom_segment(x = chronic_dpi_range$min_dpi, xend = chronic_dpi_range$max_dpi,
                         y = metrics$chronic_setpoint, yend = metrics$chronic_setpoint,
                         linetype = "solid", color = "red", linewidth = 0.6, alpha = 0.8) +
      ...
      # Add segmentation coloring
      df_with_seg <- df %>%
        left_join(seg_data %>% select(date, state_label), by = "date")
      
      gg <- gg +
        geom_point(data = df_with_seg %>% filter(!is.na(state_label)), 
                   aes(color = state_label), size = 2.5, alpha = 0.8) +
        labs(color = "Segmentation Phase") +
  ...
  if (!is.null(metrics$acute_peak)) {
    ...
    gg <- gg +
      ...
      annotate("text", x = metrics$acute_peak$day, y = metrics$acute_peak$value,
               label = "Peak", vjust = -1.2, hjust = 0.5, size = 3.2, fontface = "bold")
  }
  
  # Add only 1000 copies rebound point for ART subjects
  ...
  if (!is.null(metrics$acute_burden)) {
    if (has_art && !is.null(metrics$phases$art_start)) {
      ...
      acute_data <- df %>% filter(dpi < metrics$phases$art_start, dpi >= 0) %>% arrange(dpi)
