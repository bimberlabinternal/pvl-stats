...
CreateUnifiedTimeSeriesPlot <- function(df, metrics, has_art = FALSE) {
  ...
  if (has_art && !is.null(metrics$phases)) {
    ...
    if (!is.null(metrics$acute_setpoint) && !is.na(phases$art_start)) {
      gg <- gg +
        geom_segment(x = 0, xend = phases$art_start,
    ...
    if (!is.null(metrics$onart_setpoint)) {
      ...
      gg <- gg +
        geom_segment(x = phases$art_start, xend = art_end_x,
    ...
    if (!is.null(metrics$segmentation_raw) && !is.null(metrics$segmentation)) {
      ...
      # Add segmentation coloring but avoid final dot highlighting
      df_with_seg <- df %>%
        left_join(seg_data %>% select(date, state_label), by = "date")
      
      # Filter out the final point to avoid inappropriate highlighting
      seg_colored_points <- df_with_seg %>% 
        filter(!is.na(state_label)) %>%
        arrange(dpi) %>%
        slice(1:(n()-1))  # Remove last point
      
      if (nrow(seg_colored_points) > 0) {
        gg <- gg +
          geom_point(data = seg_colored_points, 
